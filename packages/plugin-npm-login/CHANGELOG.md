# @verdaccio/yarn-plugin-npm-login

## 0.0.10

### Patch Changes

- 7422e6d: chore: trigger release

## 0.0.9

### Patch Changes

- 0561016: Stop sending Basic authentication headers from the legacy login flow. Existing-user legacy login now retries only when the registry exposes revision metadata without Basic authentication and reports a clear error otherwise.

## 0.0.8

### Patch Changes

- ad7f789: chore: make release

## 0.0.7

### Patch Changes

- 0314723: chore: release packages

## 0.0.6

### Patch Changes

- 7366eaa: fix: update deps

## 0.0.5

### Patch Changes

- 733d2bc: Add package metadata (author, repository, bugs, homepage, license) and README to all packages. Fix login legacy flow to handle 409 Conflict matching npm adduser behaviour.

## 0.0.4

### Patch Changes

- cca5c32: fix: improve npm login

## 0.0.3

### Patch Changes

- 85205aa: fix: add npm arg flags

## 0.0.2

### Patch Changes

- a767f61: feat: add `yarn npm login` with auto/web/legacy auth flows and token persistence to `~/.yarnrc.yml`
